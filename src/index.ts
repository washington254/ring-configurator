...
async function setupViewer(){
    ...
    viewer.setBackground(new Color('#232323').convertSRGBToLinear())
    ...
    function setupScrollAnimation(){
        ...
        .fromTo(colorLerpValue, {x:0}, {x:1,
            ...
            , onUpdate: function() {
                if (!usingCustomColors) {
                    // Rose Gold
                    gold.material.color.lerpColors(new Color(0xe5adad).convertSRGBToLinear(), new Color(0xedc9af).convertSRGBToLinear(), colorLerpValue.x);
                    // Platinum
                    silver.material.color.lerpColors(new Color(0xc6c6c6).convertSRGBToLinear(), new Color(0xe1e1e1).convertSRGBToLinear(), colorLerpValue.x);
                    for (const o of diamondObjects) {
                        // Light Blue Diamond
                        o.material.color.lerpColors(new Color(0xffffff).convertSRGBToLinear(), new Color(0xa8d8ea).convertSRGBToLinear(), colorLerpValue.x);
                    }
                }
        }})
        ...
        .fromTo(colorLerpValue2, {x:0}, {x:1,
            ...
            , onUpdate: function() {
                ...
                if (!usingCustomColors) {
                    // Bronze for Gold
                    gold.material.color.lerpColors(new Color(0xd98767).convertSRGBToLinear(), new Color(0xe4b482).convertSRGBToLinear(), colorLerpValue2.x);
                    // Pearl for Silver
                    silver.material.color.lerpColors(new Color(0xe0e0e0).convertSRGBToLinear(), new Color(0xf7f7f7).convertSRGBToLinear(), colorLerpValue2.x);
                    for (const o of diamondObjects) {
                        // Pink for Diamonds
                        o.material.color.lerpColors(new Color(0xffc0cb).convertSRGBToLinear(), new Color(0xff69b4).convertSRGBToLinear(), colorLerpValue2.x);
                    }
                }
        }})
